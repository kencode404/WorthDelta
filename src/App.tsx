  ...
  return (
    <main className="auth-layout">
      <section className="auth-story" aria-labelledby="auth-title">
        <a className="brand brand-light" href={import.meta.env.BASE_URL} aria-label="WorthDelta home">
          <span className="brand-mark" aria-hidden="true">Δ</span>
          <span>WorthDelta</span>
        </a>
        <div className="story-copy">
          <p className="eyebrow">Financial clarity, month by month</p>
          <h1 id="auth-title">Know what changed.<br />Understand why.</h1>
          <p>
            Bring five years of financial history into one private, living view—and
            keep every future record close at hand.
          </p>
        </div>
        <div className="story-stat">
          <span>2022—2026</span>
          <p>Your history is prepared for a secure one-time import.</p>
        </div>
      </section>

      <section className="auth-panel" aria-labelledby="form-title">
        <div className="auth-card">
          <p className="eyebrow">{mode === 'login' ? 'Welcome back' : 'Create your private space'}</p>
          <h2 id="form-title">{mode === 'login' ? 'Sign in to WorthDelta' : 'Start tracking your worth'}</h2>
          <p className="form-intro">
            {mode === 'login'
              ? 'Your financial records are available only to your account.'
              : 'Register with email or continue directly with Google.'}
          </p>
          ...
          <form onSubmit={handleEmailAuth} noValidate>
            ...
            {error && <p className="form-alert error" role="alert"><WarningCircle aria-hidden="true" />{error}</p>}
            {message && <p className="form-alert success" role="status"><CheckCircle aria-hidden="true" />{message}</p>}

            <button className="primary-button" type="submit" disabled={loading}>
              {loading ? <SpinnerGap className="spin" aria-hidden="true" /> : <ArrowRight aria-hidden="true" />}
              {mode === 'login' ? 'Sign in' : 'Create account'}
            </button>
          </form>

          <p className="auth-switch">
            {mode === 'login' ? 'New to WorthDelta?' : 'Already have an account?'}{' '}
