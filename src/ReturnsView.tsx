...
interface CashFlowDetail { id: string; category: string; description: string; amount: number }
...
export function ReturnsView({ userId, categories, records, entries, loading }: ReturnsViewProps) {
  ...
  const rawFlows = useMemo(() => {
    ...
    const openingAssetFlows = selectedAssets.flatMap((category): Array<CashFlowDetail & { date: string }> => {
      ...
      return [{ id: `opening-${firstRecord.id}`, date: firstRecord.period, category: category.name, description: 'Opening asset value', amount: -Number(firstRecord.amount) }]
    })
